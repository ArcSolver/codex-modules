// Adapted from opencodex src/codex-home.ts and src/codex-paths.ts
import { existsSync, readFileSync, readdirSync, realpathSync, statSync } from "node:fs";
import { homedir } from "node:os";
import { join, resolve } from "node:path";
import { expandUserPath } from "./fs.js";
...
export function resolveCodexHomeDir(rawOverride?: string, deps: CodexHomeDeps = {}): string {
  const raw = rawOverride?.trim() || (deps.env ?? process.env).CODEX_HOME?.trim();
  const path = raw ? resolve(expandUserPath(raw)) : defaultCodexHome(deps);
  let stat;
  try {
    stat = statSync(path);
  } catch (error) {
    ...
    throw new Error(raw
      ? `CODEX_HOME points to ${raw}, but that path could not be read: ${message}`
      : `Codex home ${path} could not be read. Run Codex once or pass --codex-home.`);
  }
  if (!stat.isDirectory()) {
    throw new Error(raw ? `CODEX_HOME points to ${raw}, but that path is not a directory` : `${path} is not a directory`);
  }
